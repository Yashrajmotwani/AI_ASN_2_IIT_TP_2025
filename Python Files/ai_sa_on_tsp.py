...
def simulated_annealing_tsp(coords, initial_temp=1000, stopping_temp=1, alpha=0.99, epsilon=1e-6, max_iter=1000, folder='frames'):
    dist_matrix = calculate_distance_matrix(coords)
    num_cities = len(coords)

    current_tour = list(range(num_cities))
    random.shuffle(current_tour)
    assert is_valid_tsp_tour(current_tour, num_cities), "Initial tour is not valid!"
    ...
    while t >= stopping_temp and total_steps < max_iter:
        ...
        candidate_tour = get_neighbors(current_tour)
        next_tour = random.choice(candidate_tour)
        assert is_valid_tsp_tour(next_tour, num_cities), f"Invalid neighbor at iteration {total_steps}!"

        next_dist = total_distance(next_tour, dist_matrix)
        delta = next_dist - current_dist

        if abs(delta) < epsilon or delta < 0 or random.random() < math.exp(-delta / t):
...
def plot_final_tour(coords, tour, filename='final_tour.png'):
    coords_arr = np.array([coords[i] for i in tour + [tour[0]]])
    plt.figure(figsize=(6, 4))
    plt.plot(coords_arr[:, 0], coords_arr[:, 1], 'g-', marker='o')
    for i, (x, y) in enumerate(coords_arr[:-1]):
        plt.text(x, y, str(tour[i]), fontsize=8, color='red')
    ...
    plt.savefig(filename)
    plt.close()
    print(f"Final tour plot saved as {filename}")
...
def generate_gif(frame_folder='frames', output_file='simulated_annealing_tsp.gif'):
    ...
    for file in files:
        if file.endswith('.png'):
            ...
            images.append(imageio.imread(img_path))
    imageio.mimsave(output_file, images, duration=0.5)
    print(f"zGIF saved as {output_file}")
...
def plot_time_graph(times):
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(range(1, len(times) + 1), times, marker='o', linestyle='-', color='purple', label='Time per run')
    plt.axhline(y=avg_time, color='orange', linestyle='--', label=f'Average Time = {avg_time:.3f}s')
    plt.xlabel("Iteration Number")
    plt.ylabel("Time Taken (seconds)")
    plt.title("Time Taken per Simulated Annealing Run")
    plt.grid(True)
    plt.legend()
    plt.savefig("time_plot_sa.png")
    plt.close()
    print("📈 Time taken graph saved as time_plot_sa.png")

# =========================
# === RUNNING 5 TIMES ====
# =========================

tsp_file_path = '/content/ant_colony_opt_TSP/problems_cleaned/eil76.tsp'  # Change this path to your local .tsp file
coords = parse_tsp_file(tsp_file_path)

run_times = []

for run in range(1, 6):
    print(f"\nRun #{run}")
    folder = f'frames_run{run}'
    clear_frame_folder(folder)

    start_time = time.time()
    tour, dist, accepted_steps, total_steps = simulated_annealing_tsp(coords, folder=folder)
    end_time = time.time()
    ...
    plot_final_tour(coords, tour, filename=f'final_tour_run{run}.png')
    generate_gif(frame_folder=folder, output_file=f'simulated_annealing_tsp_run{run}.gif')

# Plot time taken graph
plot_time_graph(run_times)
...
def plot_tour_gif(coords, tour, step, folder='frames'):
    ...
    plt.title("Simulated Annealing Step {}".format(step))
    plt.grid(True)
    os.makedirs(folder, exist_ok=True)
...
def plot_time_graph(times):
    """
    Plots a graph of execution time per run.
    """
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(range(1, len(times) + 1), times, marker='o', linestyle='-', color='purple', label='Time per run')
    plt.axhline(y=avg_time, color='orange', linestyle='--', label='Average Time = {:.3f}s'.format(avg_time))
    plt.xlabel("Iteration Number")
    plt.ylabel("Time Taken (seconds)")
    plt.title("Time Taken per Simulated Annealing Run")
    plt.grid(True)
...
def simulated_annealing_tsp(coords, initial_temp=1000, stopping_temp=1, alpha=0.99, epsilon=1e-6, max_iter=1000, folder='frames'):
    ...
    while t >= stopping_temp and total_steps < max_iter:
        ...
        candidate_tour = get_neighbors(current_tour)
        next_tour = random.choice(candidate_tour)
        assert is_valid_tsp_tour(next_tour, num_cities), "Invalid neighbor at iteration {}!".format(total_steps)
...
for run in range(1, 6):
    ...
    folder = 'frames_run{}'.format(run)
    clear_frame_folder(folder)

    start_time = time.time()
    tour, dist, steps, total_steps = simulated_annealing_tsp(coords, folder=folder)
    end_time = time.time()
    elapsed = end_time - start_time
    run_times.append(elapsed)
    ...
    generate_gif(frame_folder=folder, output_file='sa_tsp_{}.gif'.format(run))
